...
`StableStatus` and `TitleStatus` are computed presentations of authoritative lifecycle periods; neither model persists a status column. The models expose relationship-backed state facts and delegate classification precedence to `StableStatusResolver` and `TitleStatusResolver`. Typed Actions change period records only, and callers use canonical lifecycle predicates such as `isCurrentlyActive()` and `hasFutureActivity()` instead of duplicate status aliases.
