...
Injury creation and explicit healing persistence use the typed `InjuryPeriodManager`. The database permits unlimited ended injury history while enforcing at most one open injury period per wrestler, manager, or referee. Suspension creation and explicit reinstatement persistence similarly use the typed `SuspensionPeriodManager`; the database permits unlimited ended suspension history while enforcing at most one open suspension period per wrestler, manager, referee, or tag team. Employment persistence uses the typed `EmploymentPeriodManager`, and retirement persistence uses the typed `RetirementPeriodManager`; the database permits unlimited ended retirement history while enforcing at most one open retirement period per wrestler, manager, referee, tag team, stable, or title. Concrete entity actions retain validation and effective-date resolution, then delegate only starting or ending the relevant lifecycle record. Tag-team actions also retain transaction ownership around their member cascades, and cascade collaborators invoke each related entity's complete typed action. The managers accept the relevant lifecycle contract and do not own validation, transaction orchestration, cascade behavior, or dynamic capability discovery. Release actions coordinate the relevant typed period managers directly while retaining their entity-specific cascades. Deletion actions use `DeletionPeriodCloser` to close active lifecycle dimensions while retaining transaction ownership and entity-specific relationship cleanup. With every transition migrated, `StatusTransitionPipeline` has been removed.

Retirement eligibility uses explicit model concerns rather than an operation-name string, generic `Model` contract, or dispatch-only strategy. `ValidatesIndividualRetirement` is shared only by wrestlers, managers, and referees, while `ValidatesTitleRetirement` owns title-specific rules. Each concern keeps its boolean predicates aligned with its typed throwing guards, including the rule that deleted records must be restored before unretirement. Tag teams and stables retain their established typed lifecycle validation.

Individual soft-deletion restoration eligibility is shared by wrestlers, managers, and referees through `ValidatesIndividualRestoration`. Restoration requires an existing soft-deleted record, and its boolean predicate delegates to the same typed guard used by the restoration Actions. Restoring an individual does not infer or recreate employment or other historical relationships; those transitions remain explicit operations.
...
Stable activity eligibility is isolated in `ValidatesStableActivity`. Establishment is the first activity transition and is available only to a stable with no activity history. Disbandment closes a current activity period, while reunion opens a later period for a previously active stable that satisfies the existing former-member availability rules. The concern keeps each boolean predicate aligned with its throwing guard and reports reunion failures through `CannotBeReunitedException`. `StartActivityPeriodAction` and `EndActivityPeriodAction` own the shared period mutations used by establishment, disbandment, reunion, retirement, unretirement, and merging. They lock the stable inside the transaction, reject duplicate or missing open periods, and enforce that an end date does not precede its start. Coordinating typed Actions validate eligibility after acquiring the same lock and retain their transition dates, membership consequences, and transaction boundaries. Operations involving multiple stables acquire their locks in stable-ID order to reduce deadlock risk. The `stables_activations` schema independently enforces at most one open period per stable, so direct writes and future code paths cannot bypass the Action-level invariant.

Title debut and reinstatement start activity through the title-specific `StartActivityPeriodAction`, while pulling a title ends activity through `EndActivityPeriodAction`. These actions lock the title and its open period, reject duplicate or missing current periods, validate end-date chronology, and explicitly reschedule an existing future activation without creating a second period. The `titles_activations` schema independently permits unlimited ended history while enforcing at most one open period per title.
