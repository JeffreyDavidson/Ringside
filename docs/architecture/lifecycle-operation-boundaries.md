...
`StableStatus` is a computed presentation of the authoritative activity and retirement periods; `stables` has no persisted status column. Stable Actions change period records only, and callers use canonical lifecycle predicates such as `isCurrentlyActive()` and `hasFutureActivity()` instead of duplicate status aliases.
