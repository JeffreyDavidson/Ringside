...
For example, an employed wrestler may also be injured or suspended. Active injury and suspension periods are mutually exclusive: a roster member must be cleared from injury before suspension and reinstated before injury. A single universal status enum or state machine must not erase the underlying lifecycle histories.
...
Roster booking eligibility is resolved through `RosterBookingStrategyResolver`. Individual roster members use the shared employment, suspension, and injury checks, while tag teams add their own employment, suspension, minimum-membership, and member-eligibility rules before recursively evaluating current wrestlers. The resolver is the only type-dispatch boundary; strategies remain focused on behavior and do not add booking predicates to Eloquent models.
...
Individual injury eligibility is shared only by wrestlers, managers, and referees through `IndividualInjuryEligibility`. Injury and clearance predicates delegate to the same typed guards used by their Actions, including employment, retirement, future employment, suspension, and current injury checks. Each Action reloads and locks the individual inside its transaction before checking eligibility and changing the injury period.
...
Wrestlers, managers, and referees persist injury history through the shared `App\Models\Lifecycle\Injury` model and its polymorphic `injurable` owner. `IsInjurable` exposes relationship-derived state predicates on each owner, while typed injury and clearance Actions retain validation, locking, transaction, and transition responsibilities. The database permits historical periods but enforces at most one open injury per owner.
...
The typed `StableActivityTransition` graph identifies establishment, disbandment, and reunion without executing them. `StableActivityEligibility` maps each transition to its focused guard and domain exception; the concrete Stable Actions continue to own locking, transactions, dates, membership consequences, persistence, and lifecycle auditing.
