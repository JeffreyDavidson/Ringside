...
Retirement eligibility uses explicit typed collaborators rather than an operation-name string, generic `Model` contract, or dispatch-only strategy. `IndividualRetirementEligibility` accepts only wrestlers, managers, and referees, `TagTeamRetirementEligibility` owns tag-team partner-aware rules, and `TitleLifecycleEligibility` owns title transitions. Each keeps predicates aligned with the typed guards used by Actions and is evaluated against an owner row locked inside the mutation transaction. Stables retain their established model-close validation pending a focused extraction.
