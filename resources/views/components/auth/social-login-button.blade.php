...
<a {{
    ...
    @if ($provider === 'google')
        <img
            ...
            src="{{ Vite::asset('resources/media/brand-logos/'.$config['icon']) }}"
        />
    @elseif ($provider === 'apple')
        <img
            alt="Apple"
            class="size-3.5 shrink-0 dark:hidden"
            src="{{ Vite::asset('resources/media/brand-logos/'.$config['icon_light']) }}"
        />
        <img
            alt="Apple"
            class="hidden size-3.5 shrink-0 dark:block"
            src="{{ Vite::asset('resources/media/brand-logos/'.$config['icon_dark']) }}"
        />
    @endif
    {{ $config['text'] }}
</a>
