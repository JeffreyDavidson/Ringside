...
class SuspendCurrentMembersAction
{
    ...
    public function handle(TagTeam $tagTeam, Carbon $suspensionDate): void
    {
        $wrestlers = $tagTeam->currentWrestlers()
            ->get()
            ->filter(fn (Wrestler $wrestler): bool => $wrestler->currentEmployment()->exists() && ! $wrestler->isSuspended());
        ...
        $managers = $tagTeam->currentManagers()
            ->get()
            ->filter(fn (Manager $manager): bool => $manager->currentEmployment()->exists() && ! $manager->isSuspended());
