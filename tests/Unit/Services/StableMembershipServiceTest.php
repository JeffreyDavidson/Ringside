...
it('transfers all current members while preserving source history', function () {
    $destinationStable = Stable::factory()->create();
    $wrestler = Wrestler::factory()->create();
    $tagTeam = TagTeam::factory()->create();
    $this->service->addMembers(
        $this->stable,
    ...
    $this->service->transferAllMembers($this->stable, $destinationStable, now());

    expect($this->stable->currentWrestlers()->exists())->toBeFalse()
        ->and($this->stable->currentTagTeams()->exists())->toBeFalse()
        ->and($this->stable->previousWrestlers()->whereKey($wrestler->id)->exists())->toBeTrue()
        ->and($this->stable->previousTagTeams()->whereKey($tagTeam->id)->exists())->toBeTrue()
        ->and($destinationStable->currentWrestlers()->whereKey($wrestler->id)->exists())->toBeTrue()
        ->and($destinationStable->currentTagTeams()->whereKey($tagTeam->id)->exists())->toBeTrue();
});
