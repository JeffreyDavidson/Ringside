...
test('manager index filters and table fit narrow and wide viewports', function (): void {
    ...
    $page
        ...
        ->assertScript('getComputedStyle(document.querySelector("[data-test=managers-date-range-grid]")).gridTemplateColumns.split(" ").length === 2')
        ->resize(320, 740)
        ->assertScript('getComputedStyle(document.querySelector("[data-test=managers-date-range-grid]")).gridTemplateColumns.split(" ").length === 1')
