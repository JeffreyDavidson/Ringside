...
description: Configure Laravel Nightwatch for production observability, including sampling, filtering, and redaction. Use only for production Nightwatch setup; local development uses NewDebugbar instead.
license: MIT
metadata:
  author: laravel
---
...
This skill is for the production environment only. Ringside uses NewDebugbar for local development and Nightwatch for production. Keep Nightwatch disabled locally; do not enable local collection as a way to test configuration. Configure production sampling, filtering, and redaction to balance observability, performance, and privacy.
...
Configure production via environment variables. Set `NIGHTWATCH_ENABLED=true` and provide `NIGHTWATCH_TOKEN` in the production environment. Keep `NIGHTWATCH_ENABLED=false` in local development (the project `.env.example` default).

```bash

# Package defaults are 100%; set production rates deliberately.
...
**Recommendation**: Start with `0.1` (10%) for production requests, then adjust based on volume and needs.
