...
This reference applies to production only. Keep `NIGHTWATCH_ENABLED=false` locally; use NewDebugbar for local development. Enable Nightwatch and provide its token only in the production environment.
...
- [ ] Confirm production sampling rates are intentional before deployment
